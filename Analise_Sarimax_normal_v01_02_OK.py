...
class ModeloSARIMAXMelhorado:
    ...
    def _validate_input_data(self, df: pd.DataFrame, target_col: str, exog_cols: List[str]) -> None:
        ...
        if missing_exog:
            raise ValueError(
                f"Colunas exógenas não encontradas: {missing_exog}")
    ...
    def detectar_outliers_avancado(self, methods: List[str] = ['iqr', 'zscore', 'isolation'],
                                   threshold_zscore: float = 3.0) -> Dict:
        """
        ...
        if 'iqr' in methods:
            ...
            outliers_iqr = series[(series < lower_bound)
                                  | (series > upper_bound)]
        ...
        if 'zscore' in methods:
            ...
            outliers_zscore = series[np.abs(
                modified_z_scores) > threshold_zscore]
        ...
        # Ordenar por impacto
        self.outlier_dates = sorted(self.outlier_dates,
    ...
    def criar_features_avancadas(self,
                                 ...
                                 include_cyclical: bool = True) -> Tuple[pd.DataFrame, List[str]]:
        """
        ...
        df_features['fim_semana'] = (
            df_features.index.dayofweek >= 5).astype(int)
        df_features['segunda_feira'] = (
            df_features.index.dayofweek == 0).astype(int)
        df_features['sexta_feira'] = (
            df_features.index.dayofweek == 4).astype(int)
        br_holidays = holidays.Brazil(years=range(
            self.df.index.year.min(), self.df.index.year.max() + 1))
        df_features['feriado'] = df_features.index.to_series().isin(
            br_holidays).astype(int)

        # Períodos do mês
        df_features['inicio_mes'] = (df_features.index.day <= 5).astype(int)
        df_features['meio_mes'] = ((df_features.index.day > 10) & (
            df_features.index.day <= 20)).astype(int)
        ...
        if include_cyclical:
            ...
            df_features['feriado'] = df_features.index.strftime(
                '%Y-%m-%d').isin(feriados).astype(int)
        ...
        features_temporais = [col for col in df_features.columns
            ...
            if col in df_features.columns:
                ...
                for window in [7, 30]:
                    if window < len(serie_exog):
                        ...
                        df_features[ratio_col] = serie_exog / \
                            (ma_values + 1e-8)
        ...
        if include_interactions and len(self.original_exog_cols) >= 2:
            ...
            for i, col1 in enumerate(self.original_exog_cols):
                for col2 in self.original_exog_cols[i+1:]:
                    ...
                    if 'SALARY' in df_features.columns and 'RESCISSION' in df_features.columns:
                    ...
                    if col1 in df_features.columns and col2 in df_features.columns:
                        ...
                        df_features[interaction_col] = df_features[col1] * \
                            df_features[col2]
                        df_features[ratio_col] = df_features[col1] / \
                            (df_features[col2] + 1e-8)
                        df_features[diff_col] = df_features[col1] - \
                            df_features[col2]

                        features_criadas.extend(
                            [interaction_col, ratio_col, diff_col])
        ...
        if include_technical:
            ...
            for window in [7, 14, 30]:
                if window < len(target_series):
                    ...
                    df_features[momentum_col] = target_series - \
        ...
        if self.outlier_dates:
            ...
            df_features['evento_pandemia'] = (
                (df_features.index >= '2020-03-01') & (df_features.index <= '2020-06-30')).astype(int)
        ...
        df_features['regime_level_alto'] = (
            target_series > level_threshold).astype(int)
    ...
    def selecao_features_inteligente(self,
                                     df_features: pd.DataFrame,
                                     max_features: int = 10,
                                     methods: List[str] = ['mutual_info', 'correlation', 'random_forest']) -> List[str]:
        """
        ...
        exclude_cols = [self.target_col, 'Emprestimo'] + \
            self.original_exog_cols
        ...
        if 'mutual_info' in methods:
            try:
                mi_scores = mutual_info_regression(
                    features_data, target_series,
                    random_state=self.random_state
                )
                rankings['mutual_info'] = pd.Series(
                    mi_scores, index=features_data.columns).rank(ascending=False)
                if self.verbose:
        ...
        if 'random_forest' in methods:
            try:
                ...
                rf_features = features_data.iloc[:, :min(
                    50, len(features_data.columns))]
        ...
        if 'f_score' in methods:
            try:
                ...
                f_scores = pd.Series(f_selector.scores_,
        ...
        while len(selected_features) < max_features and remaining_candidates:
            ...
            if selected_features:
                correlations = features_data[selected_features +
                                             [candidate]].corr()
                max_corr = correlations.loc[candidate,
                                            selected_features].abs().max()
    ...
    def grid_search_otimizado(self,
                              df_features: pd.DataFrame,
                              selected_features: List[str],
                              test_seasonal: bool = False,
                              max_models: int = 50) -> Tuple[Optional[object], List[Dict]]:
        """
        ...
        if test_seasonal and self.seasonal_info.get('has_seasonality', False):
            ...
            if test_seasonal and self.seasonal_info.get('has_seasonality', False):
                ...
                seasonal_orders = [(1, 0, 1, p) for p in periods_to_test] + \
                    [(1, 1, 1, p) for p in periods_to_test] + \
                    [(0, 0, 0, 0)]

            # Gerar todas as combinações
            combinations = list(itertools.product(
                p_range, d_range, q_range, seasonal_orders))
            np.random.shuffle(combinations)
            logger.info(
                f"   • Incluindo sazonalidade nos períodos: {periods_to_test}")
        else:
            seasonal_orders = [(0, 0, 0, 0)]
        ...
        total_combinations = len(p_range) * len(d_range) * \
            len(q_range) * len(seasonal_orders)
        ...
        for p, d, q, seasonal_order in combinations:
            ...
            try:
                ...
                try:
                    ljung_box = acorr_ljungbox(residuos.dropna(), lags=min(
                        10, len(residuos)//5), return_df=True)
                ...
                resultados.append({
                    ...
                    'complexity_penalty': len(fitted_model.params) / len(target_series)
                })

                if self.verbose and models_tested % 10 == 0:
                    status = "✓" if converged and not problema_coef else "⚠"
                    logger.info(f"   {status} [{models_tested}/{min(total_combinations, max_models)}] "
        ...
        if self.verbose:
            ...
            logger.info(
                f"      Quality Score: {melhor_modelo['quality_score']:.3f}")
            logger.info(
    ...
    def validacao_walk_forward_robusta(self,
                                       df_features: pd.DataFrame,
                                       n_splits: int = 5,
                                       min_train_ratio: float = 0.6) -> Optional[Dict]:
        """
        ...
        if self.selected_features:
            exog_data = df_features[self.selected_features].reindex(
                target_series.index)
            exog_data = exog_data.fillna(exog_data.median())
            exog_scaled = pd.DataFrame(
        ...
        metricas = {
            ...
            'mase': [], 'direcao_correta': [], 'max_erro': [], 'bias': []
        }
        ...
        for fold in range(n_splits):
            ...
            try:
                ...
                common_idx = test_original.index.intersection(
                    pred_original.index)
                if len(common_idx) == 0:
                    continue
                ...
                if self.verbose:
                    logger.info(f"   Fold {fold+1}: RMSE={metrics_fold['rmse']:,.0f}, "
        ...
        for metrica, valores in metricas.items():
            valores_validos = [
                v for v in valores if not np.isnan(v) and np.isfinite(v)]
            if valores_validos:
        ...
        rmse_cv = resultados_consolidados['rmse']['std'] / \
            resultados_consolidados['rmse']['media']
        mae_cv = resultados_consolidados['mae']['std'] / \
    ...
    def _calculate_comprehensive_metrics(self, y_true: pd.Series, y_pred: pd.Series, train_series: pd.Series) -> Dict:
        """
        ...
        try:
            ...
            if len(y_true) > 1:
                ...
                if len(y_true_diff) > 0:
                    direction_correct = np.sum(np.sign(y_true_diff) == np.sign(
                        y_pred_diff)) / len(y_true_diff) * 100
    ...
    def diagnosticos_modelo_completos(self) -> Dict:
        ...
        try:
            shapiro_stat, shapiro_p = stats.shapiro(residuos.dropna())
            jarque_bera_stat, jarque_bera_p = stats.jarque_bera(
                residuos.dropna())
        ...
        try:
            ljung_box = acorr_ljungbox(residuos.dropna(), lags=min(
                20, len(residuos)//5), return_df=True)
        ...
        if diagnosticos.get('heterocedasticidade', {}).get('has_heteroscedasticity', False):
        ...
        if self.verbose:
            ...
            if problemas:
                logger.warning(
                    f"   ⚠️ Problemas identificados: {', '.join(problemas)}")
            else:
    ...
    def executar_analise_completa(self,
                                  max_features: int = 10,
                                  test_seasonal: bool = False,
                                  n_splits_validation: int = 5) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if self.verbose:
                print("Antes do tratamento de NaNs:\n",
            ...
            if self.verbose:
                print("Depois do tratamento de NaNs:\n",
                      df_features.isnull().sum()[df_features.isnull().any()])
            ...
            if not selected_features:
                logger.error(
                    "❌ Nenhuma feature selecionada. Análise interrompida.")
                return None
    ...
    def _gerar_relatorio_final_avancado(self, resultados: Dict) -> None:
        ...
        if self.best_model:
            ...
            seasonal = self.best_model.specification.get(
                'seasonal_order', (0, 0, 0, 0))
            logger.info(f"   • Modelo selecionado: SARIMAX{ordem}{seasonal}")
            logger.info(f"   • AIC: {self.best_aic:.3f}")
            logger.info(
                f"   • Features utilizadas: {len(self.selected_features)}")
            logger.info(
        ...
        if self.validation_results and 'metricas' in self.validation_results:
            ...
            melhoria_rmse = ((rmse_original - rmse_atual) /
                             rmse_original) * 100
            ...
            if melhoria_rmse > 10 and melhoria_mae > 10:
                logger.info(
                    f"   🎉 SUCESSO EXCEPCIONAL! Melhoria significativa alcançada!")
            elif melhoria_rmse > 5 and melhoria_mae > 5:
                logger.info(
                    f"   🎉 SUCESSO! Modelo substancialmente melhorado!")
            elif melhoria_rmse > 0 and melhoria_mae > 0:
                logger.info(f"   👍 Melhoria positiva alcançada!")
            else:
                logger.info(f"   ⚠️ Modelo necessita ajustes adicionais")
        ...
        if 'diagnosticos' in resultados and resultados['diagnosticos']:
            ...
            if not problemas:
                logger.info(
                    "   ✅ Modelo passou em todos os testes diagnósticos")
            else:
        ...
        if len(self.selected_features) > 5:
            logger.info(
                f"   ... e mais {len(self.selected_features) - 5} features")
        ...
        if 'outliers' in resultados and resultados['outliers']:
            ...
            if n_outliers > 0:
                ...
                for i, date in enumerate(outliers_info['all_outliers'][:3], 1):
                    impact = outliers_info.get(
                        'impact_scores', {}).get(date, 0)
                    logger.info(
        ...
        if self.validation_results and 'metricas' in self.validation_results:
            rmse_atual = self.validation_results['metricas']['rmse']['media']
            stability = self.validation_results.get('stability_score', 0)

            if rmse_atual < 1000000 and stability > 0.7:
                logger.info(
                    f"   ✅ Modelo com excelente performance e estabilidade. Pronto para produção!")
                logger.info(f"   📝 Próximos passos:")
                logger.info(f"     • Implementar em ambiente de produção")
                logger.info(f"     • Configurar monitoramento contínuo")
                logger.info(f"     • Agendar retreinamento mensal")

            elif rmse_atual < 1200000:
                logger.info(f"   👍 Modelo com boa performance. Considere:")
                logger.info(f"     • Coletar dados exógenos adicionais")
                logger.info(f"     • Testar ensemble com outros algoritmos")
                logger.info(f"     • Refinir tratamento de outliers")

            else:
                logger.info(f"   ⚠️ Modelo precisa de melhorias:")
                logger.info(f"     • Revisar qualidade e completude dos dados")
                logger.info(
                    f"     • Investigar variáveis explicativas ausentes")
                logger.info(f"     • Considerar modelos alternativos (ML)")

        # Baseadas nos diagnósticos
        if 'diagnosticos' in resultados:
            problemas = resultados['diagnosticos'].get(
                'problemas_identificados', [])
            if problemas:
                logger.info(f"\n   🔧 Melhorias específicas recomendadas:")
                if 'Resíduos não normais' in problemas:
                    logger.info(
                        f"     • Investigar transformações adicionais do target")
                if 'Autocorrelação nos resíduos' in problemas:
                    logger.info(f"     • Considerar termos AR/MA adicionais")
                if 'Heterocedasticidade' in problemas:
                    logger.info(
                        f"     • Avaliar modelos GARCH para volatilidade")

        # 8. INFORMAÇÕES TÉCNICAS
        logger.info(f"\n🔧 DETALHES TÉCNICOS:")
        logger.info(f"   • Método de seleção: Ensemble (MI + Correlação + RF)")
        logger.info(f"   • Escalonamento: RobustScaler")
        logger.info(
            f"   • Validação: Walk-Forward ({self.validation_results.get('n_folds_successful', 0)} folds)")
        logger.info(f"   • Critério de seleção: AIC composto")
        logger.info(
    ...
    def gerar_previsoes_melhoradas(self,
                                   dias_futuro: int = 7,
        ...
        try:
            ...
            exog_futuras = self._preparar_exogenas_futuras_avancado(
                datas_futuras)
            ...
            if include_confidence:
                ...
                if self.verbose:
                    logger.info(
                        "✅ Previsões com intervalos de confiança geradas")
                    logger.info(
                        f"   • Score de incerteza: {uncertainty_score:.3f}")
            ...
            if self.verbose:
                ...
                for i, (data, valor) in enumerate(pred_original.items()):
                    ...
                    if include_confidence:
                        ...
                        logger.info(f"   {data.strftime('%Y-%m-%d')} ({dia_semana}): "
    ...
    def _preparar_exogenas_futuras_avancado(self, datas_futuras: pd.DatetimeIndex) -> Optional[pd.DataFrame]:
        """
        ...
        for feature in self.selected_features:
            try:
                # Features temporais
                if 'dia_semana' in feature and 'sin' not in feature and 'cos' not in feature:
                    exog_futuras[feature] = datas_futuras.dayofweek
                elif 'fim_semana' in feature:
                    exog_futuras[feature] = (
                        datas_futuras.dayofweek >= 5).astype(int)
                elif 'inicio_mes' in feature:
                    exog_futuras[feature] = (
                        datas_futuras.day <= 5).astype(int)
                elif 'meio_mes' in feature:
                    exog_futuras[feature] = ((datas_futuras.day > 10) & (
                        datas_futuras.day <= 20)).astype(int)
                elif 'fim_mes' in feature:
                    exog_futuras[feature] = (
                        datas_futuras.day >= 25).astype(int)
                elif 'segunda_feira' in feature:
                    exog_futuras[feature] = (
                        datas_futuras.dayofweek == 0).astype(int)
                elif 'sexta_feira' in feature:
                    exog_futuras[feature] = (
                        datas_futuras.dayofweek == 4).astype(int)
                ...
                # Features cíclicas
                    ...
                    exog_futuras[feature] = np.sin(
                        2 * np.pi * datas_futuras.month / 12)
                ...
                # Features das exógenas originais
                    ...
                    for col_orig in self.original_exog_cols:
                        if col_orig in feature and col_orig in self.df_features_engineered.columns:
                            # Para lags, usar valores históricos se disponível
                            if '_lag_' in feature:
                                lag_num = int(feature.split('_lag_')[1])
                                valores_historicos = self.df_features_engineered[col_orig].dropna(
                                )
                                if len(valores_historicos) >= lag_num:
                                    # Usar valores com lag apropriado
                                    exog_futuras[feature] = valores_historicos.iloc[-lag_num]
                                else:
                                    exog_futuras[feature] = valores_historicos.iloc[-1]
                            else:
                                # Para outras features, usar último valor
                                ultimo_valor = self.df_features_engineered[col_orig].dropna(
                                ).iloc[-1]
                ...
                # Features de regime (usar último valor conhecido)
                    ...
                    if feature in self.df_features_engineered.columns:
                        ultimo_valor = self.df_features_engineered[feature].dropna(
                        ).iloc[-1]
                        exog_futuras[feature] = ultimo_valor
                    else:
                        exog_futuras[feature] = 0
    ...
    def _analisar_contexto_previsoes(self, previsoes: pd.Series, datas: pd.DatetimeIndex) -> Dict:
        """
        ...
        try:
            ...
            # Tendência geral
            if len(previsoes) > 1:
                tendencia = np.polyfit(range(len(previsoes)),
                                       previsoes.values, 1)[0]
                contexto['tendencia'] = {
                    'slope': tendencia,
                    'direcao': 'crescente' if tendencia > 0 else 'decrescente' if tendencia < 0 else 'estável'
                }
            ...
            contexto['vs_historico'] = {
                ...
                'diferenca_percentual': ((previsoes.mean() - media_historica) / media_historica) * 100
            }
    ...
    def exportar_dados_dashboard(self, caminho_json: str = 'dashboard_data.json') -> bool:
        """
        ...
        try:
            ...
            if self.validation_results and 'metricas' in self.validation_results:
                ...
                dashboard_data["metricas"] = {
                    "rmse": {
                        ...
                        "melhoria": round(((rmse_original - metricas['rmse']['media']) / rmse_original) * 100, 1)
                    },
                    "mae": {
                        "atual": int(metricas['mae']['media']),
                        "std": int(metricas['mae']['std']),
                        "original": mae_original,
                        "melhoria": round(((mae_original - metricas['mae']['media']) / mae_original) * 100, 1)
                    },
                    "mape": {
                        "atual": round(metricas['mape']['media'], 1),
                        "std": round(metricas['mape']['std'], 1),
                        "original": mape_original,
                        "melhoria": round(((mape_original - metricas['mape']['media']) / mape_original) * 100, 1)
                    },
                    "r2": {
                        "atual": round(metricas['r2']['media'], 2),
                        "std": round(metricas['r2']['std'], 3),
                        "original": r2_original,
                        "melhoria": round(((metricas['r2']['media'] - r2_original) / r2_original) * 100, 1)
                    }
                }
            ...
            if self.selected_features and hasattr(self, 'feature_importance_scores'):
                for i, feature in enumerate(self.selected_features):
                    ...
                    if any(temp in feature.lower() for temp in ['dia_', 'mes_', 'fim_', 'inicio_']):
                        tipo = 'temporal'
                    elif any(col.lower() in feature.lower() for col in self.original_exog_cols):
                        tipo = 'exog'
            ...
            if hasattr(self, 'df') and self.target_col in self.df.columns:
                serie_data = self.df[self.target_col].dropna().tail(
                    30)  # Últimos 30 dias
            ...
            dashboard_data["configuracao"] = {
                ...
                "features_selecionadas": len(self.selected_features) if self.selected_features else 0,
                "periodo_inicio": self.df.index.min().strftime('%Y-%m-%d') if hasattr(self, 'df') else None,
                "periodo_fim": self.df.index.max().strftime('%Y-%m-%d') if hasattr(self, 'df') else None,
            ...
            if self.verbose:
                logger.info(
                    f"✅ Dados exportados com sucesso para {caminho_json}")
                logger.info(
                    f"   • Métricas: {len(dashboard_data['metricas'])} grupos")
                logger.info(
                    f"   • Features: {len(dashboard_data['features'])}")
                logger.info(
        ...
        for i, modelo_alt in enumerate(modelos_alternativos):
            if self.validation_results:
                ...
                modelos_comparacao.append({
                    ...
                    "mae": int(self.validation_results['metricas']['mae']['media'] * fator_penalidade),
                    "r2": round(self.validation_results['metricas']['r2']['media'] / fator_penalidade, 3),
                    "melhor": False,
                    "convergiu": True,
...
def executar_analise_sarimax_completa(
    ...
    try:
        ...
        if len(valores_validos) == 0:
            raise ValueError(
                "Nenhum valor válido encontrado na coluna Emprestimo")
        # --- Transformação alternativa do target (exemplo: raiz quadrada) ---
        df_historico[target_col] = np.sqrt(df_historico['Emprestimo'])
        ...
        # Tratar zeros
        df_historico[target_col] = np.log(
            df_historico['Emprestimo'].replace(0, np.nan))
        ...
        try:
            ...
            df_historico[target_col +
                         "_boxcox"], _ = stats.boxcox(df_historico[target_col] + 10)
        ...
        if len(df_historico) < 30:
            raise ValueError(
                "Dados insuficientes para análise (mínimo 30 observações)")
    ...
    try:
        ...
        if salvar_modelo_path:
            ...
            if not sucesso_salvamento:
                logger.warning(
                    "⚠️ Erro ao salvar modelo, mas análise continua...")
        ...
        resultado_final = {
            ...
            'metricas_resumo': {
                'rmse': modelo.validation_results['metricas']['rmse']['media'] if modelo.validation_results else None,
                'mae': modelo.validation_results['metricas']['mae']['media'] if modelo.validation_results else None,
                'r2': modelo.validation_results['metricas']['r2']['media'] if modelo.validation_results else None,
                'stability_score': modelo.validation_results['stability_score'] if modelo.validation_results else None
            },
...
def comparar_modelos(resultados_lista: List[Dict], nomes: List[str] = None) -> pd.DataFrame:
    """
...
def gerar_relatorio_html(resultado: Dict, caminho_saida: str = 'relatorio_sarimax.html') -> bool:
    """
...
def otimizar_hiperparametros(df_historico: pd.DataFrame,
                             target_col: str = 'Log_Emprestimo',
                             original_exog_cols: List[str] = [
                                 'SALARY', 'RESCISSION'],
    ...
    for max_features in max_features_range:
        ...
        try:
            ...
            if verbose:
                logger.warning(
                    f"   ❌ Erro com max_features={max_features}: {e}")
            continue
    ...
    if verbose:
        ...
        if melhores_params:
            logger.info(
                f"   • max_features: {melhores_params['max_features']}")
...
if __name__ == "__main__":
    ...
    if resultados_finais:
        ...
        try:
            df_opt = pd.read_csv('base_historica.csv', delimiter=';', decimal=',',
            ...
            df_opt['Emprestimo'] = pd.to_numeric(
                df_opt['Emprestimo'], errors='coerce')
            df_opt['Log_Emprestimo'] = np.log(
                df_opt['Emprestimo'].replace(0, np.nan))
            df_opt.dropna(subset=['Log_Emprestimo'], inplace=True)

            otimizacao = otimizar_hiperparametros(
                df_historico=df_opt,
                max_features_range=[6, 8, 10, 12, 15],
                verbose=True
            )
        ...
        sucesso_html = gerar_relatorio_html(
            ...
            caminho_saida='relatorio_sarimax_completo.html'
        )
        ...
        print(f"\n📁 Arquivos gerados:")
        print(f"   • modelo_sarimax_melhorado.joblib")
        print(f"   • relatorio_sarimax_completo.html")
        print(f"   • sarimax_analysis.log")

        print(f"\n🎉 ANÁLISE COMPLETA FINALIZADA COM SUCESSO!")
        print(f"✨ Use 'resultados_finais' para acessar todos os resultados.")
